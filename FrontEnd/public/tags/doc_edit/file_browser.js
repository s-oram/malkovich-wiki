riot.tag('docedit_filebrowser', ' <ul class="FileItemList"> <li if="{ DataStores.PageFiles.FileList.length == 0 }" class="NoFilesFound"> No files found </li> <li each="{ DataStores.PageFiles.FileList }" class="FileItem" draggable="true" data-dragtype="File" data-pageid="{DataStores.PageData.PageID}" data-filename="{name}" data-wikilink="{wikilink}" > <div class="FileIcon"></div> <div class="FileName">{ name }</div> <div class="FileButtons"> <button class="FileDeleteButton" data-pageid="{DataStores.PageData.PageID}" data-filename="{name}" >&nbsp;</button> </div> </li> </ul>', 'class="docedit_filebrowser"', function(opts) {
"use strict";
var self = this;
self.opts = opts;
